...
Edit any playbook/*.md, run this, and get:
  0 START HERE.pdf            (top of the folder) sheet 1
  5 How-To Guides/            sheets 2-4 as PDFs, friendly names
...
TOP_DIR = PLAYBOOK_DIR.parent.parent  # the folder a person actually opens
OUT_DIR = TOP_DIR / "5 How-To Guides"
...
def md_to_html(md_text: str, title: str) -> str:
    ...
    return (
        ...
        f"to change this sheet, ask Claude</div>"
        f"</body></html>"
    )


def pdf_path(stem: str) -> Path:
    """1-START-HERE -> '0 START HERE.pdf' at the top; 2-MAKE-A-GRAPHIC -> 'Make a Graphic.pdf'."""
    if stem.startswith("1-"):
        return TOP_DIR / "0 START HERE.pdf"
    return OUT_DIR / (stem.split("-", 1)[1].replace("-", " ").title().replace(" A ", " a ") + ".pdf")
...
def main() -> int:
    ...
    print(f"\nDone. {len(sheets)} sheets. Print from 0 START HERE.pdf and 5 How-To Guides/")
    return 0
