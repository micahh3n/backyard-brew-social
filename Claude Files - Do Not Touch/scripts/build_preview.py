"""
build_preview.py - Generates preview/this-week.html (inside the Claude Files folder), a static visual
...
PREVIEW_DIR = os.path.join(config.SYSTEM_DIR, "preview")
# preview/ sits two levels below the top folder that generated_image paths are relative to
TOP = "../../"
...
def _card_html(row: dict) -> str:
    ...
    img_tag = (f'<img src="{TOP}{html.escape(img_rel)}" alt="post image">'
               if img_rel else '<div class="placeholder"></div>')
    extra_photos = [p.strip() for p in (row.get("photos") or "").split(",")[1:] if p.strip()]
    extra_html = ""
    if extra_photos:
        imgs = "".join(f'<img src="{TOP}3 Photos/{html.escape(p)}" alt="additional photo">'
